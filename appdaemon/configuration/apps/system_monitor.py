...
class NotifyOnLowBattery(AppBase):
    ...
    APP_SCHEMA = APP_SCHEMA.extend(
        {
            ...
            CONF_PROPERTIES: vol.Schema(
                {vol.Optional(CONF_BATTERY_LOW_THRESHOLD): int}, extra=vol.ALLOW_EXTRA
            ),
        }
    )
    ...
    def configure(self) -> None:
        """Configure."""
        for entity in self.entities[CONF_TRACKING_DEVICES]:
            self.listen_state(self.battery_low, entity, constrain_app_enabled=1)
        ...
        for subdirname in subdirs:
            if "appdaemon" in subdirname.lower():
                if "info" in subdirname.lower():
                    ...
                    self.set_state("sensor.appdaemon_installed", state=version)
...
class NotifyOnNewVersion(AppBase):
    ...
    def configure(self) -> None:
        ...
        for entity in self.entities[CONF_AVAILABLE]:
            self.listen_state(self.version_changed, entity, constrain_app_enabled=1)
