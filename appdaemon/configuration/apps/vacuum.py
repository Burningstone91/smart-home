...
class VacuumAutomation(AppBase):
    ...
    APP_SCHEMA = APP_SCHEMA.extend(
        {
            CONF_ENTITIES: vol.Schema(
                {vol.Required(VACUUM): vol_help.entity_id}, extra=vol.ALLOW_EXTRA
            ),
            CONF_PROPERTIES: vol.Schema(
                {vol.Optional(CONF_CLEANING_TIME): str}, extra=vol.ALLOW_EXTRA
            ),
    ...
    def configure(self) -> None:
        ...
        self.run_daily(self.start_cleaning, cleaning_time, constrain_app_enabled=1)
        ...
        self.listen_state(
            self.set_cleaning_mode_input_boolean, self.vacuum, attribute=STATUS
        )
    ...
    def cancel_cleaning(
        ...
        if (
            not self.presence_app.noone_home and self.started_by_app
    ...
    def set_cleaning_mode_input_boolean(
        self, entity: Union[str, dict], attribute: str, old: str, new: str, kwargs: dict
    ) -> None:
        ...
        if "cleaning_mode" in MODES and old != new:
            ...
            elif new in (self.VacuumState.charging.value, self.VacuumState.stuck.value):
